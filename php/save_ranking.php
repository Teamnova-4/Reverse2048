...
try {
    ...
    if (empty($data['gameSize'])) {
        $missingFields[] = 'gameSize';
    }
    ...
    // 랭킹 데이터 삽입
    $stmt = $pdo->prepare("INSERT INTO rankings (nickname, turn, play_time, mapSize) VALUES (?, ?, ?, ?)");
    $stmt->execute([$data['nickname'], $data['turn'], $data['formattedTime'], $data['gameSize']]);
